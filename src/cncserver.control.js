...
module.exports = (cncserver) => {
  ...
  cncserver.control.setPen = (inPen, callback) => {
    ...
    if (typeof inPen.power !== 'undefined') {
      const powers = cncserver.botConf.get('penpower');
      if (typeof powers === 'undefined') {
        // We have no super powers
        powers = { min: 0, max: 0 }; // Set the powers to zero
      }

      cncserver.run(
        'custom',
        cncserver.control.cmdstr('penpower', {
    ...
    if (inPen.x !== undefined) {
      ...
      const penNaN = isNaN(inPen.x) || isNaN(inPen.y);
      const penFinite = isFinite(inPen.x) && isFinite(inPen.y);
      if (penNaN || !penFinite) {
        callback(false);
        return;
      }
      ...
      const absInput = cncserver.utils.centToSteps(inPen);
  ...
  cncserver.control.setHeight = (state, callback, skipBuffer) => {
    ...
    if (cncserver.pen.height) {
      ...
      const range = parseInt(servo.max) - parseInt(servo.min);
      servoDuration =
        Math.round(
  ...
  cncserver.control.setTool = (toolName, callback, ignoreTimeout) => {
    ...
    const downHeight = toolName.indexOf('water') !== -1 ? 'wash' : 'draw';
    ...
    if (typeof tool.wait !== 'undefined') {
      ...
      // "Standard" WaterColorBot toolchange
    ...
    if (callback) {
      if (!ignoreTimeout) {
        ...
        // Run as soon as items have been buffered
        callback(1);
      }
    }
  ...
  cncserver.control.movePenAbs = (inPoint, callback, immediate, skip) => {
    ...
    const point = Object.assign({}, inPoint);
    ...
    const startOffCanvasChange = false;
    ...
    if (callback) {
      if (immediate === true) {
        ...
        if (cmdDuration < 2) {
          ...
          setTimeout(() => callback(cncserver.pen), cmdDuration);
        }
      }
    }
  ...
  cncserver.control.offCanvasChange = (newValue) => {
    ...
    if (cncserver.pen.offCanvas !== newValue) {
      ...
      if (cncserver.pen.offCanvas) {
        // Pen is now off screen/out of bounds
        ...
        // Pen is now back in bounds
        // Set the state regardless of actual change
        const back = cncserver.pen.state;
  ...
  cncserver.control.actuallyMove = (destination, callback) => {
    ...
    if (cncserver.buffer.data.length === 0) {
      cncserver.pen = Object.assign({}, cncserver.actualPen);
    }
  ...
  cncserver.control.wigglePen = (axis, travel, iterations) => {
    ...
    function _wiggleSlave(toggle) {
      ...
      if (i <= iterations) {
        ...
        // Done wiggling, go back to start
