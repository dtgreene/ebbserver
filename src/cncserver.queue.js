...
module.exports = function (cncserver) {
  ...
  cncserver.buffer.pause = function () {
    ...
    cncserver.buffer.pausePen = Object.assign({}, cncserver.actualPen);
  ...
  cncserver.buffer.startItem = function (hash) {
    ...
    if (cncserver.buffer.dataSet[hash] && index > -1) {
      ...
      cncserver.actualPen = Object.assign({}, item.pen);
  ...
  cncserver.run = function (command, data, duration) {
    ...
    cncserver.buffer.addItem({
      command: c,
      duration: duration,
      pen: Object.assign({}, cncserver.pen),
    });
    ...
    if (typeof item.command === 'object') {
      ...
      switch (item.command.type) {
        case 'absmove':
          ...
          break;
        case 'absheight':
          const hChange = cncserver.utils.getHeightChangeData(
            item.command.source,
            item.command.z,
          );
          commandOut = [
            cncserver.buffer.cmdstr('movez', { z: item.command.z }),
          ];

          // If there's a togglez, run it after setting Z
          if (cncserver.bot.commands.togglez) {
            commandOut.push(
              cncserver.buffer.cmdstr('togglez', {
  ...
  cncserver.buffer.trigger = function (item) {
    if (typeof item.command === 'function') {
      ...
      item.command(1);
      return true;
    } else if (typeof item.command === 'object') {
      // Detailed buffer object
      switch (item.command.type) {
        case 'message':
